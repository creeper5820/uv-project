 ...
 *
 * Note:
 *  - Memory for ETH DMA Descriptors (DMARxDscrTab and DMATxDscrTab) must be
 *    configured as: non-cacheable, non-shareable device memory, execute never
 *  - Memory for ETH frame buffers (Rx_Buff and Tx_Buff) must be configured as:
...
-----------------
  1. Under Connectivity open \b ETH Configuration:
     ...
     - <b>Parameter Settings</b>: configure descriptor and RX buffer memory locations
          ...
          Rx Buffers Address             | \b 0x30040100
          Rx Buffers Length              | \b 1524

     \n\note
     Add <b>"RxDecripSection", "TxDecripSection", "RxArraySection" and "TxArraySection" </b> sections to the Scatter file if GNU Compiler or Arm Compiler 6 is used.\n
     Example:
     \code{.sct}
  ...
  2. Under System open \b CORTEX_M7 Configuration
     - <b>Parameter Settings</b>: optionally enable cache and configure MPU
          ...
          Cortex MPU Region 1 Settings | Value
          :----------------------------|:-------
          MPU Region                   | \b Enabled
          MPU Region Base Address      | \b 0x30040000
          MPU Region Size              | \b 32kB
          MPU SubRegion Disable        | \b 0x0
          MPU TEX field level          | \b level 1
          MPU Access Permission        | \b ALL ACCESS PERMITTED
          MPU Instruction Access       | \b DISABLE
          MPU Shareability Permission  | \b DISABLE
          MPU Cacheable Permission     | \b DISABLE
          MPU Bufferable Permission    | \b DISABLE

          Cortex MPU Region 2 Settings | Value
          :----------------------------|:-------
          MPU Region                   | \b Enabled
          MPU Region Base Address      | \b 0x30040000
...
/* Driver version */
#define ARM_ETH_MAC_DRV_VERSION         ARM_DRIVER_VERSION_MAJOR_MINOR(1,5)
...
__attribute__((at(0x30041900))) static uint8_t Tx_Buff[ETH_TX_DESC_CNT][ETH_MAX_PACKET_SIZE]; /* Ethernet Transmit Buffer */
